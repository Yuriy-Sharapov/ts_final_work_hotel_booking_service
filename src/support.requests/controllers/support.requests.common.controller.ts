...
export class SupportRequestCommonController {
    ...
    @Get('/:id/messages')
    async getMessages(@Param() { id }: IParamId): Promise<ISupportRequestMessageAnswer[]> {
    ...
    async readMessages(
        ...
        switch (user.role) {
            case Role.manager:
                return this.supportRequestsEmployeeService.markMessagesAsRead(markMessagesAsReadDto)
            case Role.client:
                return this.supportRequestsClientService.markMessagesAsRead(markMessagesAsReadDto)              
        }       
    }
