...
export class SupportRequestManagerController {
    ...
    @Get()
    async getSupportRequests(@Query() params: ISearchSupportRequestParams): Promise<ISupportRequestManagerAnswer[]> {
        ...
        for (let i = 0; i < supportRequests.length; i++){
            ...
            if (await this.supportRequestsEmployeeService.getUnreadCount(supportRequest.id) > 0) {
            ...
            const user = await this.usersService.findByEmail(supportRequest.user.email)
