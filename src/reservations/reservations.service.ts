  ...
  async addReservation(data: IReservationDto): Promise<ReservationDocument> {
    ...
    const checkAlreadyBookedConditions = {
      $and: [
        ...
        {
          $ne: {
            // делаем отрицание того, когда существующие бронирования не пересекаются с запрашиваемым
            $or: [
              // описываем условия, когда существующие бронирования не пересекаются с запрашиваемым
    ...
    if (!alreadyBooked) {
      ...
      console.log(
        `Номер на даты ${userDateStart} - ${userdateEnd} уже забронирован`,
      );
      return null;
    }
